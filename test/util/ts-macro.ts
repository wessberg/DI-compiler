...
export type ExtendedImplementation = (
  t: ExecutionContext,
  options: ExtendedImplementationArgumentOptions
) => void | Promise<void>;
...
noMatchingVersionMacro.title = (provided = "") =>
  `${provided} (No matching TypeScript versions)`;
...
for (const [specifier, range] of Object.entries(devDependencies)) {
  ...
  if (match !== null) {
    ...
    if (context === "npm:typescript@" || specifier === "typescript") {
      ...
      if (
        filter === undefined ||
        (filter.toUpperCase() === "CURRENT" && specifier === "typescript") ||
        satisfies(version, filter, { includePrerelease: true })
      ) {
...
export function withTypeScriptVersions(
  extraFilter: string
): OneOrMoreMacros<[ExtendedImplementation], unknown> {
  const filteredMacros = [...macros.entries()]
    .filter(([version]) =>
      satisfies(version, extraFilter, { includePrerelease: true })
    )
    .map(([, macro]) => macro);
