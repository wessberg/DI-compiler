...
import {FormattedParameter, IFormattedClass} from "@wessberg/type";
...
export class ClassConstructorArgumentsStringifier implements IClassConstructorArgumentsStringifier {
	 ...
	 * @returns {string}
	 */
	public getClassConstructorArgumentsStringified (classes: IFormattedClass[], mappedInterfaces: IMappedInterfaceToImplementationMap): string {
		...
		keys.forEach((key, index) => {
			...
			const classDeclaration = classes.find(declaration => declaration.name != null && declaration.name.name === className);
			const mappedArguments = classDeclaration == null || classDeclaration.constructor == null ? [] : classDeclaration.constructor.parameters.map(parameter => this.normalizeConstructorParameter(parameter, mappedInterfaces)).join(", ");
	 ...
	 * @param {FormattedParameter} parameter
	 * @param {IMappedInterfaceToImplementationMap} mappedInterfaces
	 * @returns {string}
	 */
	private normalizeConstructorParameter (parameter: FormattedParameter, mappedInterfaces: IMappedInterfaceToImplementationMap): string {
		if (parameter.type == null || mappedInterfaces[parameter.type.toString()] == null) return "undefined";
