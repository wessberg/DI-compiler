...
export class ClassConstructorArgumentsValidator implements IClassConstructorArgumentsValidator {
	 ...
	 */
	public validate (classes: IFormattedClass[], mappedInterfaces: IMappedInterfaceToImplementationMap): void {
		classes.forEach(declaration => {
			if (declaration.constructor == null) return;

			declaration.constructor.parameters.forEach((parameter, index) => {
				if (parameter.type.toString() == null && parameter.initializer == null) {
					// If neither type nor initializer is given, throw an error. We must know if the argument should be dependency injected.
					throw new TypeError(`${this.constructor.name} could not validate the class: ${declaration.name}: The constructor argument "${parameter.toString()}" on argument position ${index} had no type declaration and no initializer. You must either give it a type so it can be dependency injected or initialize it yourself to a value!`);
				}

				if (parameter.type.toString() != null && parameter.initializer == null && mappedInterfaces[parameter.type.toString()] == null) {
					// If it has a declared type that isn't initialized and that isn't registered as a service, throw an error.
					throw new ReferenceError(`${this.constructor.name} could not validate the class: ${declaration.name}: The constructor argument: "${parameter.toString()}" on argument position ${index} could not be found. Remember to register it as a service or initialize it to a value from the argument position!`);
				}
			});
		});
